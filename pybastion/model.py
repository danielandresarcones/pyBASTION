"""
Main public API for pyBASTION.

Provides ``fit_BASTION``, the entry point for Bayesian time-series
decomposition.
"""

import numpy as np

from .mcmc import fit_ASD_SV
        ...
        if i == 0:
            ...
            if reg and "reg_coef" in model["samples"]:
                combined_samples["reg_coef"] = np.concatenate(
                    [combined_samples["reg_coef"], model["samples"]["reg_coef"]],
                    axis=0,
                )
